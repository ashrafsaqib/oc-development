      ...
      <div class="panel-heading" style="overflow: hidden;">
        <div class="pull-left">
          <h3 class="panel-title" style="margin: 0; line-height: 30px;">
            <a data-toggle="collapse" href="#feed-settings" class="collapsed">
              <i class="fa fa-rss"></i> <?php echo $text_feed_settings; ?> <i class="fa fa-caret-down"></i>
            </a>
          </h3>
        </div>
      </div>
      <div id="feed-settings" class="panel-collapse collapse">
        <div class="panel-body">
          <div class="row">
            <div class="col-sm-6">
              <div class="form-group">
                <label class="control-label"><?php echo $entry_feed_status; ?></label>
                <div>
                  <label class="radio-inline">
                    <input type="radio" name="feed_status" value="1" <?php echo $feed_status ? 'checked' : ''; ?> /> <?php echo $text_enabled; ?>
                  </label>
                  <label class="radio-inline">
                    <input type="radio" name="feed_status" value="0" <?php echo !$feed_status ? 'checked' : ''; ?> /> <?php echo $text_disabled; ?>
                  </label>
                </div>
              </div>
              <div class="form-group">
                <label class="control-label" for="input-feed-format"><?php echo $entry_feed_format; ?></label>
                <select name="feed_format" id="input-feed-format" class="form-control">
                  <option value="json" <?php echo $feed_format == 'json' ? 'selected' : ''; ?>><?php echo $text_format_json; ?></option>
                  <option value="xml" <?php echo $feed_format == 'xml' ? 'selected' : ''; ?>><?php echo $text_format_xml; ?></option>
                </select>
              </div>
              <div class="form-group">
                <label class="control-label" for="input-feed-limit"><?php echo $entry_feed_limit; ?></label>
                <input type="text" name="feed_limit" value="<?php echo $feed_limit; ?>" id="input-feed-limit" class="form-control" />
              </div>
            </div>
            <div class="col-sm-6">
              <div class="form-group">
                <label class="control-label" for="input-feed-cache"><?php echo $entry_feed_cache; ?></label>
                <input type="text" name="feed_cache" value="<?php echo $feed_cache; ?>" id="input-feed-cache" class="form-control" />
                <span class="help-block"><?php echo $help_feed_cache; ?></span>
              </div>
              <div class="form-group">
                <label class="control-label"><?php echo $entry_feed_url; ?></label>
                <div class="input-group">
                  <input type="text" value="<?php echo $feed_url; ?>" readonly class="form-control" id="feed-url-input" />
                  <span class="input-group-btn">
                    <button type="button" class="btn btn-default" onclick="copyFeedUrl()"><i class="fa fa-copy"></i></button>
                  </span>
                </div>
                <span class="help-block"><?php echo $help_feed_url; ?></span>
              </div>
              <div class="form-group">
                <button type="button" id="button-save-feed" class="btn btn-primary"><i class="fa fa-save"></i> <?php echo $button_save_feed; ?></button>
                <button type="button" id="button-clear-cache" class="btn btn-warning"><i class="fa fa-refresh"></i> <?php echo $button_clear_cache; ?></button>
              </div>
            </div>
          </div>
        </div>
...
function copyFeedUrl() {
	var copyText = document.getElementById('feed-url-input');
	copyText.select();
...
// Save feed settings
$('#button-save-feed').on('click', function() {
	$.ajax({
		url: 'index.php?route=extension/module/customer_export/saveFeed&token=<?php echo $token; ?>',
		type: 'post',
		data: {
			feed_status: $('input[name="feed_status"]:checked').val(),
			feed_format: $('select[name="feed_format"]').val(),
			feed_limit: $('input[name="feed_limit"]').val(),
			feed_cache: $('input[name="feed_cache"]').val()
		},
		dataType: 'json',
		beforeSend: function() {
			$('#button-save-feed').button('loading');
		},
		complete: function() {
			$('#button-save-feed').button('reset');
		},
		success: function(json) {
			if (json['success']) {
				alert(json['success']);
...
$('#button-clear-cache').on('click', function() {
	if (confirm('Are you sure you want to clear the feed cache?')) {
		$.ajax({
			url: 'index.php?route=extension/module/customer_export/clearCache&token=<?php echo $token; ?>',
			type: 'post',
			dataType: 'json',
			beforeSend: function() {
