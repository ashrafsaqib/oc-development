...
class ControllerExtensionInstaller extends Controller {
				...
				if (file_exists($file)) {
					...
					if ($zip) {
						...
						$json['step'][] = array(
							...
							'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/copy', 'token=' . $this->session->data['token'], true)),
							'path' => $path
						);
	...
	public function ftp() {
		// Redirect to the new copy method for backward compatibility
		return $this->copy();
	}
	...
	public function copy() {
		...
		if (!$json) {
			// Get a list of files ready to copy
			$files = array();
			...
			// Copy files using local file system
			foreach ($files as $file) {
				$destination = substr($file, strlen($directory));

				// Determine the correct destination path
				// Many people rename their admin folder for security purposes
				// The following code allows you to change the name of directories and extensions will still go to the right directory
				if (substr($destination, 0, 5) == 'admin') {
					...
					$destination = dirname(DIR_APPLICATION) . '/' . $destination;
				}
				...
				if (is_file($file)) {
					...
					if (!is_dir($dest_dir)) {
						if (!mkdir($dest_dir, 0755, true)) {
							$json['error'] = sprintf($this->language->get('error_ftp_directory'), $dest_dir);
							break;
						}
					}
